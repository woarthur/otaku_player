            ...
            using (CloudMusicApi api = new CloudMusicApi())
            {
                try
                {
                    ...
                    foreach (var t in songsTemp)
                    {
                        ...
                        int id1 = (int)t["id"];
                        (isOk1, json1) = await api.RequestAsync(CloudMusicApiProviders.Lyric, new Dictionary<string, string> { { "id", id1.ToString() } });
                        if (!isOk1)
                            throw new ApplicationException($"获取歌词信息失败： {json1}");
                        var lrc = json1;
        ...
        public async Task<(ArtistInfo ArtistInfo, List<MusicInfo> HotSongs)> GetArtistInfoAndHotSongs(int artistNo)
        {
            ...
            using (CloudMusicApi api = new CloudMusicApi())
            {
                try
                {
                    ...
                    {
                        (isOk1, json1) = await api.RequestAsync(CloudMusicApiProviders.Lyric, new Dictionary<string, string> { { "id", song.id.ToString() } });
                        if (!isOk1)
                            throw new ApplicationException($"获取歌词信息失败： {json1}");
                        var lrc = json1;
